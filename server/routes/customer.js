...
router.put('/profile', (request, response) => {
  ...
  db.execute(
    statement,
    [name, phone, address, request.user.id],
    (error, result) => {
      response.send(utils.createResponse(error, result))
    }
  )
  ...
  const statement = `
    delete from customer
        where
            id = ?
    `
  db.execute(statement, [request.user.id], (error, result) => {
    response.send(utils.createResponse(error, result))
  })
})
...
router.get('/profile', (request, response) => {
  ...
  const statement = `
    select 
        name, phone, email, address from customer 
    where 
        id = ?
  `
  db.execute(statement, [request.user.id], (error, result) => {
...
router.post('/login', (request, response) => {
  ...
  db.execute(statement, [email, encryptedPassword], (error, users) => {
    if (error) {
      ...
      if (users.length == 0) {
        // no user found
        response.send(utils.createErrorResponse('customer not found'))
      } else {
        const user = users[0]
